...
def color_region(img, rgb_threshold):
...
def draw_lines(img, lines, color, thickness=12):
    """
...
def process_image(image_ori):
    ...
    image = np.copy(image_ori)  # creating a blank to draw lines on
...
imageInputFolder = "test_images/"
imageOutputFolder = "test_images_output/"
fileLists = os.listdir(imageInputFolder)
RoI_Vertices = [[100, 540], [960, 540], [480, 310]]  # Left(x, y)  Right(x, y)  Top(x, y)
RGB_Threshold_White_Line = [200, 200, 200]  # Define color criteria [red_threshold, green_threshold, blue_threshold]
RGB_Threshold_All_Line = [200, 190, 90]  # Define color criteria [red_threshold, green_threshold, blue_threshold]
color_Green = [0, 255, 0]  # Mark White Lines as Green, Can Pass.
color_Red = [255, 0, 0]  # Mark Yellow Lines as Red, Can not Pass.
...
for n in fileLists:
    ...
    # printing out some stats and plotting
    print('This image is:', n, 'with dimensions:', image_ori.shape)
